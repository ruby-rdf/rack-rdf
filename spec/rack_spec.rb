...
describe Rack::RDF do
  ...
  def app
    ...
    @app ||= Rack::RDF::ContentNegotiation.new(target_app, @options)
  end
  ...
  context "serializes graphs" do
    ...
    context "with format" do
      %w(ntriples nquads).map(&:to_sym).each do |fmt|
        context fmt do
    ...
    context "with Accept" do
      ...
      context "with profile accept-param" do
        let(:header) {%(application/n-triples;profile="http://www.w3.org/ns/json-ld#compacted http://example.org/white-listed")}
